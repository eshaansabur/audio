...
const AllUsers = () => {
    ...
    return (
        ...
        <p className='text-success'>Total Users: {users.length}, Active Users: {activeUsers.length}, Inactive Users: {inactiveUsers.length}</p>
        ...
        <tbody>
            {
                users.map(user => <AllUser key={user._id} user={user} handleDeleteUser={handleDeleteUser}></AllUser>)
            }
        </tbody>
        </table>   
        </div>
